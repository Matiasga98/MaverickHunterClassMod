...
{
    internal class ThirdCharge : ModProjectile
    {
                ...
                case AIState.Shooting:
                    {
                        ...
                        else if (charge < 160)
                        {
                            if (Main.myPlayer == Projectile.owner)
                            {
                                ...
                                Projectile.NewProjectile(Projectile.InheritSource(Projectile), newCenter, Projectile.velocity * 1.1f, ModContent.ProjectileType<ThirdBuster1>(), Projectile.damage + 30, Projectile.knockBack, Main.myPlayer);
                                busterPlayer.isCharging = false;
                                Projectile.Kill();
                            }
                        }
                        else if (charge < chargeLimit)
                        {
                            ...
                            {
                                //Vector2 newCenter = new Vector2(Projectile.Center.X + 61, Projectile.Center.Y);
                                Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center, Projectile.velocity *0.2f, ModContent.ProjectileType<ThirdBuster2>(), Projectile.damage + 30, Projectile.knockBack, Main.myPlayer);
                                busterPlayer.isCharging = false;
                                busterPlayer.stockChargeThird = 1;
                                Projectile.Kill();
                            }
                        }
                        else
                        {
                            if (Main.myPlayer == Projectile.owner)
                            {
                                ...
                                Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center, Projectile.velocity * 0.2f, ModContent.ProjectileType<ThirdBuster2>(), Projectile.damage + 30, Projectile.knockBack, Main.myPlayer);
                                busterPlayer.isCharging = false;
                                busterPlayer.stockChargeThird = 2;
                                Projectile.Kill();
                            }
                        }
                        break;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            if(charge < 20)
            {
                ...
                int startX = fullCharge ? frameWidth * 3 : charge > 160? frameWidth * 2 : charge > 100? frameWidth*1:0;
                Rectangle sourceRectangle = new Rectangle(startX, startY, texture.Width/4, frameHeight);
                Vector2 origin = sourceRectangle.Size() / 2f;
                float offsetX = 1f;
                origin.X =  (float)(Projectile.spriteDirection == 1 ? sourceRectangle.Width - offsetX : offsetX);
                Color drawColor = Projectile.GetAlpha(lightColor);
                Main.EntitySpriteDraw(texture,
                    Projectile.Center - Main.screenPosition + new Vector2(texture.Width/8, Projectile.gfxOffY),
