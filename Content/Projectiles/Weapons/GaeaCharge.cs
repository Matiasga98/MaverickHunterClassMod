...
{
    internal class GaeaCharge : ModProjectile
    {
                ...
                case AIState.Shooting:
                    {
                        ...
                        if (charge < 20)
                        {
                            if (Main.myPlayer == Projectile.owner)
                            {
                                ...
                                Projectile.NewProjectile(Projectile.InheritSource(Projectile), newCenter, Projectile.velocity, ModContent.ProjectileType<BusterShot2>(), Projectile.damage + 5, Projectile.knockBack, Main.myPlayer);
                                busterPlayer.isCharging = false;
                                Projectile.Kill();
                            }
                        }
                        ...
                        else
                        {
                            if (Main.myPlayer == Projectile.owner)
                            {
                                ...
                                Projectile.NewProjectile(Projectile.InheritSource(Projectile), newCenter, Projectile.velocity * 1.1f, ModContent.ProjectileType<GaeaBusterShot>(), Projectile.damage + 30, Projectile.knockBack, Main.myPlayer);
                                busterPlayer.isCharging = false;
                                Projectile.Kill();
                            }
                        }
